...
ask_input() {
  ...
  read -rp "$prompt ${def:+($def)}: " ans || true
  echo "${ans:-$def}"
}
need_root() {
  if [[ $EUID -ne 0 ]]; then
...
current_login_user() {
  # prefer SUDO_USER if present, else whoami
...
# Parse optional flags (kept for power users)
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      cat <<EOF
Usage: sudo bash install.sh [--install|--uninstall] [--user <name>] [--ssids "ssid1,ssid2"] [--debounce <seconds>]
...
if [[ -z "${SSIDS}" ]]; then
  ...
  if [[ -n "$cur_ssid" ]]; then
    ...
    echo "No current Wi-Fi SSID detected."
  fi
